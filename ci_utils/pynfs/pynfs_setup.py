...
class PyNFSManager:
    def __init__(self, session, server_ip, repo_url="git://git.linux-nfs.org/projects/cdmackay/pynfs.git"):
        ...
        Args:
            ...
            repo_dir: Directory where pynfs repo will be cloned/built.
        """
    ...
    def clone_and_build(self) -> None:
        """Clone pynfs repo and build it."""
        logger.info("Cloning and building pynfs...")
    ...
    def run_test(
        ...
        """
        logger.info("Running pynfs tests for NFSv%s...", version)
    ...
    # def collect_failures(self, outputs: List[str]) -> bool:
    def collect_failures(self, outputs: List[Tuple[str, str, int]]) -> Tuple[bool, str]:
        """
        Parse captured outputs and collect failures into a summary string.

        Args:
            outputs: List of command outputs (strings) from pynfs runs.

        Returns:
            True if failures found, False otherwise.
        """
        logger.info("Collecting pynfs failures...")
        fail_found = False
        failure_summary = []
        summary_text = ""
        return_code = 0
    ...
    def run_all_tests(self, export) -> bool:
        """
        Run pynfs tests for 4.0 and 4.1.

        Args:
            server: NFS server IP.
            export: Export path.
            test_parameters: Extra args for NFSv4.0 run.
        ...
        logger.info("Running all pynfs test suites...")
