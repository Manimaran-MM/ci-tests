...
class SpectrumScaleInstaller:
    ...
    def setup_spectrumscale(self):
        ...
        run_cmd(self.session, f"{self.spectrum_scale_binary} node add {self.vm_ip} -n")
        run_cmd(self.session, f"{self.spectrum_scale_binary} node add {self.vm_ip} -p")
        run_cmd(self.session, f"{self.spectrum_scale_binary} config protocols -e {self.usable_ip}")
        run_cmd(self.session, f"{self.spectrum_scale_binary} node add -a {self.vm_ip}")
        run_cmd(self.session, f"{self.spectrum_scale_binary} config gpfs -c {self.vm_ip}_cluster")
    ...
    def setup_storage(self):
        ...
        run_cmd(self.session, f"{self.spectrum_scale_binary} nsd add -p {self.vm_ip} -u dataAndMetadata -fs scale_volume -fg 1 /home/nsd1_c84f2u09-rhel88a1")
