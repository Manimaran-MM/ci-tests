...
class SpectrumScaleInstaller:
    def __init__(self, session, username, password, vm_ip, path_version_to_use, workspace, ssh_key, nodes: dict = None, system_type="centos", installer_http_path=None, export_name="/ibm/fs1"):
        ...
        self.nodes = nodes or {"admin": [vm_ip], "servers": [vm_ip], "clients": []}
    ...
    def install_packages(self):
        ...
        def _install_on_node(node, sess):
            ...
            if platform == "centos":
                run_cmd(sess, "wget https://kojihub.stream.centos.org/kojifiles/packages/kernel/5.14.0/522.el9/x86_64/kernel-devel-5.14.0-522.el9.x86_64.rpm")
                run_cmd(sess, "wget https://kojihub.stream.centos.org/kojifiles/packages/kernel/5.14.0/522.el9/x86_64/kernel-headers-5.14.0-522.el9.x86_64.rpm")
                run_cmd(sess, "ls -la")
                run_cmd(sess, "dnf -y install openssl-fips-provider ./kernel-devel-5.14.0-522.el9.x86_64.rpm ./kernel-headers-5.14.0-522.el9.x86_64.rpm")
            elif platform == "rhel":
    ...
    def configure_spectrumscale(self):
        ...
        run_cmd(
            self.session,
            # f"{self.spectrum_scale_binary} node add {admin_node} -a -m -n -p",
    ...
    def setup_storage(self):
        ...
        if self.system_type == "baremetal":
            ...
            for node in servers:
                ...
                if self.system_type == "openstack" and len(servers) == 1:
                    # Single-node OpenStack → same disk for metadata & data
                    disk = usable_disks[0]
                    nsd_map[node] = {"metadata": [disk], "data": [disk]}
                else:
                    # Multi-node (baremetal or openstack)
                    nsd_map[node] = {
        ...
        # --------------------------
            ...
            # for node, sess in self.node_sessions["servers"].items():
                logger.info(f"Creating disks on {node}")
                disks = []

                # Create 2 disks of 10GB each (you can change these values)
                for i in range(1, 3):
