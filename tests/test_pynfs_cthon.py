...
"""
        ...
        # Dedicated session for PyNFS so we can abort tests without closing server_session (jump host for vm_session).
        pynfs_session = RemoteSession(
            node_ip=server_node,
            user="root",
            default_dir=server_workspace,
        )
        pynfs_session.connect()
        try:
            pynfs = PyNFSManager(session=pynfs_session, server_ip=vm_ip, backend_type="gpfs")
            result_holder = [None]
            ganesha_died = threading.Event()
            test_done = threading.Event()
            ...
            if ganesha_stopped:
                logger.error("Ganesha process died during test; test run aborted.")
                pynfs_log = pynfs.fetch_run_log(session=server_session)
                if pynfs_log:
                    test_context = PyNFSManager.parse_last_test_context(pynfs_log)
                    logger.error("PyNFS context at ganesha death:\n%s", test_context)
                    pynfs_log_file = os.path.join(FAILURE_FILE, "pynfs_gpfs_testserver.log")
                    with open(pynfs_log_file, "w", encoding="utf-8") as f:
                        f.write(pynfs_log)
                    logger.info("PyNFS testserver log written to %s", pynfs_log_file)
                    failure_summary = (
                        f"{failure_summary}\n\n--- Ganesha died during PyNFS ---\n{test_context}"
                        if failure_summary
                        else f"Ganesha died during PyNFS\n{test_context}"
                    )
            ...
            if fail_found or ganesha_stopped:
                # Ganesha runs in VM; use vm_session for backtrace via server_session jump host.
