...
def all_nodes(request):
    ...
    # session_file = BAREMETAL_SESSION_FILE if baremetal else SESSION_FILE
...
def test_pynfs_gpfs(create_session):
    ...
    gerrit_refspec = os.getenv("GERRIT_REFSPEC", "refs/changes/30/1219130/10")
...
"""
    ...
    pynfs = PyNFSManager(session=server_session, server_ip=server_node)
