...
def create_sessions(request, nodes):
    ...
    # all_nodes = nodes["admin"] + nodes["servers"][1:] + nodes["clients"]
...
def installer(request, nodes, create_sessions):
    ...
    # # Normalize to list of tuples if accidentally wrapped in a tuple
    # if isinstance(create_sessions, tuple) and len(create_sessions) == 1 and isinstance(create_sessions[0], list):
    #     session_list = create_sessions[0]
    # else:
    #     session_list = create_sessions

    node_sessions = {ip: session for session, _, ip in create_sessions}
    # node_sessions = {ip: session for session, _, ip in session_list}
