...
def test_cthon(remote_sessions, reserved_nodes):
    ...
    assert rc == 0, "Cthon CephFS tests failed"
...
def test_pjdfs(remote_sessions, reserved_nodes):
    logger.info("[TEST START]: PJDFS with CephFS")
    ...
    _, failure_summary, code = pjdfs.run_all_tests(export="/nfs/cephfs")
    logger.info("PJDFS test failure summary: %s", failure_summary)

    assert code == 0, "PJDFS CephFS tests failed"
