...
# ---------------------------------------------------------
def gerrit_custom_message(return_code, test_name):
    if return_code == 0:
        failure_msg = f"\n**🟢 {test_name}:** `Passed`"
...
def test_checkpatch(create_session, server_node):
    ...
    files_to_copy = [
        ...
        # f"{WORKSPACE}/ci-tests/build_scripts/common/check-patch.sh"
    ]
    ...
    out, code = run_cmd(remote_session, cmd, check=False)

    gerrit_custom_message(code, "Checkpatch lint")

    if "Checkpatch OK" not in out or code != 0:
        checkpatch_log_file = os.path.join(FAILURE_FILE, "checkpatch_logs.txt")
...
def test_clang_format(create_session):
    ...
    gerrit_custom_message(code, "Clang-format Check")
    if code:
